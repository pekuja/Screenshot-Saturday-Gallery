var SqueezeBox = {
	...
	close : function(e) {
		...
		if(stoppable) 
			e.stop();
		if(!this.isOpen || (stoppable && !$lambda(this.options.closable).call(this, e))) 
			return this;
	...
	resize : function(size, instantly) {
		...
		var to = {
			...
			top : (scroll.y + (box.y - 75 - this.size.y - this.options.marginInner.y) / 2).toInt()
		};
		this.hideContent();
		if(!instantly) {
	...
	reposition : function() {
		...
		this.win.setStyles({
				...
				top : ((scroll.y + (size.y - this.win.offsetHeight) / 2) - 35).toInt() + 'px'
			});
...
SqueezeBox.parsers.extend({
		image : function(preset) {
			return(preset || (/\.(?:jpg|jpeg|png|gif)$/i).test(this.url)) ? this.url : false;
		},
...
SqueezeBox.handlers.extend({
		image : function(url) {
			...
			tmp.onload = tmp.onabort = tmp.onerror = (function() {
					...
					this.asset.addEvent('click', function() { SqueezeBox.close(); } );
